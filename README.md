# easemob-callkit-signaflow

环信CallKit信令跟踪可视化工具库，用于解析和可视化CallKit信令日志，生成清晰的序列图和报告。

## 功能特性

- 📊 **信令日志解析**：自动解析环信CallKit生成的信令日志文件
- 📈 **可视化报告**：生成HTML格式的可视化报告，包含通话列表和信令序列图
- 🧩 **Mermaid支持**：支持生成Mermaid格式的序列图数据
- 🖥️ **Web服务**：提供Web界面，支持上传日志文件生成报告
- 📱 **命令行工具**：提供CLI接口，支持批量处理日志文件
- 🏷️ **通话分类**：自动识别一对一、群组和未知类型的通话
- 🔍 **快速导航**：通过通话ID列表快速定位到特定通话详情

## 项目结构

```
easemob-callkit-signaflow/
├── src/                          # TypeScript源码目录
│   ├── core/                     # 核心逻辑模块
│   │   ├── Types.ts              # 信令数据类型定义
│   │   ├── SignalParser.ts       # 日志解析器（输入log→输出信令事件）
│   │   └── Visualizer.ts         # 可视化生成器（信令事件→HTML/Mermaid）
│   ├── app.ts                    # Express服务（提供交互式Web界面）
│   └── index.ts                  # CLI命令入口（用户执行入口）
├── logInput/                     # 示例日志文件目录
│   └── easemob.log               # 示例信令日志
├── package.json                  # 项目配置（含npm脚本和依赖）
├── tsconfig.json                 # TypeScript配置（类型检查规则）
└── README.md                     # 项目说明（含使用示例）
```

## 安装

### 前提条件

- Node.js 18.x 或更高版本
- npm 或 yarn 包管理器

### 安装步骤

1. 克隆项目到本地

```bash
git clone https://github.com/Easemob-Community/easemob-callkit-signaflow.git
cd easemob-callkit-signaflow
```

2. 安装依赖

```bash
npm install
```

3. 构建项目

```bash
npm run build
```

## 使用指南

### 1. 命令行工具（CLI）

#### 解析日志文件

```bash
node dist/index.js parse <logFile> [options]
```

**参数说明**：
- `<logFile>`: 信令日志文件路径
- `-o, --output <outputFile>`: 输出文件路径（可选）

**示例**：
```bash
node dist/index.js parse logInput/easemob.log -o parsed-data.json
```

#### 生成可视化报告

```bash
node dist/index.js visualize <logFile> [options]
```

**参数说明**：
- `<logFile>`: 信令日志文件路径
- `-f, --format <format>`: 报告格式（html 或 mermaid，默认：html）
- `-o, --output <outputFile>`: 输出文件路径（可选）

**示例**：
```bash
# 生成HTML报告
node dist/index.js visualize logInput/easemob.log -o report.html

# 生成Mermaid数据文件
node dist/index.js visualize logInput/easemob.log -f mermaid -o sequence.mmd
```

#### 将Mermaid数据转换为HTML

```bash
node dist/index.js mermaid-to-html <mermaidFile> [options]
```

**参数说明**：
- `<mermaidFile>`: Mermaid数据文件路径
- `-o, --output <outputFile>`: 输出HTML文件路径（可选）

**示例**：
```bash
node dist/index.js mermaid-to-html sequence.mmd -o mermaid-report.html
```

#### 启动Web服务

```bash
node dist/index.js serve [options]
```

**参数说明**：
- `-p, --port <port>`: 服务端口（默认：3000）

**示例**：
```bash
node dist/index.js serve -p 8080
```

### 2. Web服务

启动Web服务后，通过浏览器访问 `http://localhost:3000`，可以使用以下功能：

1. **上传日志文件**：选择信令日志文件
2. **选择报告格式**：HTML或Mermaid
3. **生成报告**：点击按钮生成可视化报告
4. **下载报告**：自动下载生成的报告文件

## 可视化报告说明

生成的HTML报告包含以下内容：

### 1. 统计信息

- 总通话数
- 一对一通话数
- 群组通话数
- 未知类型通话数

### 2. 通话ID列表

- 显示所有通话的CallID
- 每个CallID前显示通话类型标识：
  - 【一对一】：绿色标识
  - 【群组】：蓝色标识
  - 【未知】：红色标识
- 点击CallID可快速跳转到对应通话详情

### 3. 通话详情

每个通话详情包含：
- 信令序列图（使用Mermaid生成）
- 详细信令日志列表
- 原始日志内容

## API接口

### Web服务API

#### 健康检查

```
GET /health
```

**响应**：
```json
{
  "status": "ok",
  "timestamp": "2025-12-18T00:00:00.000Z"
}
```

#### 解析日志

```
POST /api/parse
```

**请求体**：
```json
{
  "logContent": "日志内容"
}
```

**响应**：
```json
{
  "message": "解析API已创建，具体实现将在后续添加",
  "received": {"logContent": "日志内容"}
}
```

#### 生成可视化报告

```
POST /api/visualize
```

**请求体**：
```json
{
  "logContent": "日志内容",
  "format": "html"
}
```

**响应**：
```json
{
  "message": "可视化API已创建，具体实现将在后续添加",
  "received": {"format": "html"}
}
```

#### Mermaid转HTML

```
POST /api/mermaid-to-html
```

**请求体**：
```json
{
  "mermaidContent": "sequenceDiagram...",
  "outputFileName": "report.html"
}
```

**响应**：HTML文件下载

## 开发指南

### 安装依赖

```bash
npm install
```

### 构建项目

```bash
npm run build
```

### 类型检查

```bash
npx tsc --noEmit
```

### 开发模式（监听文件变化）

```bash
npx tsc --watch
```

### npm脚本说明

| 脚本命令                | 说明                                  |
|-------------------------|---------------------------------------|
| `npm run build`         | 编译TypeScript代码到dist目录          |
| `npm run preprocess`    | 运行日志预处理脚本                    |
| `npm run preprocess:dev`| 编译并运行日志预处理脚本              |
| `npm run parse`         | 运行日志解析脚本                      |
| `npm run parse:dev`     | 编译并运行日志解析脚本                |
| `npm run process`       | 运行数据处理脚本                      |
| `npm run process:dev`   | 编译并运行数据处理脚本                |
| `npm test`              | 运行测试（当前未实现具体测试用例）    |

## 技术栈

- **TypeScript**: 类型安全的JavaScript超集
- **Node.js**: 服务器端JavaScript运行时
- **Express**: Web应用框架
- **Mermaid.js**: 序列图生成库
- **Commander.js**: CLI命令行工具
- **fs-extra**: 文件系统操作库

## 许可证

ISC License

## 贡献

欢迎提交Issue和Pull Request！

## 联系方式

如有问题，请通过以下方式联系我们：

- GitHub Issues: https://github.com/Easemob-Community/easemob-callkit-signaflow/issues
- 环信官网: https://www.easemob.com/

## 更新日志

### v1.0.0 (2025-12-17)

- 初始版本发布
- 支持解析环信CallKit信令日志
- 支持生成HTML和Mermaid格式的可视化报告
- 提供CLI和Web服务两种使用方式
- 自动识别通话类型并分类显示
- 支持通过通话ID列表快速导航